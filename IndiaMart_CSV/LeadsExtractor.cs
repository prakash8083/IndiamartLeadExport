...
namespace IndiaMart_CSV
{
    public partial class LeadsExtractor : Form
    {
        ...
        private async void btnExport_Click(object sender, EventArgs e)
        {
            ...
            string data = "";
            data += "QUERY_ID,QTYPE,SENDERNAME,SENDEREMAIL,SUBJECT,DATE_RE,DATE_R,DATE_TIME_RE,MOB,COUNTRY_FLAG,QUERY_MODID,ENQ_MESSAGE,ENQ_ADDRESS,ENQ_CITY,ENQ_STATE,PRODUCT_NAME,EMAIL_ALT,MOBILE_ALT,PHONE,PHONE_ALT,TOTAL_COUNT,GLUSR_USR_COMPANYNAME,ENQ_CALL_DURATION,ENQ_RECEIVER_MOB" + Environment.NewLine;
            foreach (var item in leads)
            {
                data += "\""+item.QUERY_ID + "\",\""  + item.QTYPE + "\",\""  + item.SENDERNAME + "\",\"" + item.SENDEREMAIL + "\",\""  + item.SUBJECT + "\",\""  + item.DATE_RE + "\",\""  + item.DATE_R + "\",\""  + item.DATE_TIME_RE + "\",\""  +
                    ...
                    item.ENQ_STATE + "\",\""  + item.PRODUCT_NAME + "\",\""  + item.EMAIL_ALT + "\",\"" + ReplaceStringLiteral(item.MOBILE_ALT) + "\",\"" + ReplaceStringLiteral(item.PHONE) + "\",\""  + ReplaceStringLiteral(item.PHONE_ALT) + "\",\""  + item.TOTAL_COUNT + "\",\"" + item.GLUSR_USR_COMPANYNAME + "\",\"" + item.ENQ_CALL_DURATION + "\",\"" + item.ENQ_RECEIVER_MOB + "\""+Environment.NewLine;
